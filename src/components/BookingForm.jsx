...
export default function BookingForm({ currentUser, details, goBack, onSubmit, selectedHall, capacity }) {
    ...
    const handleSubmit = (e) => {
        ...
        const newBooking = {
            id: Date.now(),
            title: formData.get('title'), hall: selectedHall, date: details.date,
            startTime: details.startTime, endTime: details.endTime, status: 'pending',
            requestedBy: formData.get('organiserName'), email: formData.get('email'),
            designation: formData.get('designation'), department: formData.get('department'),
            phone: '', purpose: formData.get('purpose'),
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto bg-white dark:bg-gray-800 p-8 rounded-lg shadow-xl">
            <div className="flex items-center mb-6">
                <button onClick={goBack} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 mr-4"><ArrowLeft size={24} /></button>
            ...
            <form onSubmit={handleSubmit} className="space-y-4" noValidate>
                ...
                <div>
                    ...
                    <textarea id="purpose" name="purpose" rows="3" required className={inputClasses} placeholder="Describe the main objective..."></textarea>
                ...
                <div className="flex items-center gap-2 mt-4">
                    <input id="guest-check" type="checkbox" checked={isGuestLecture} onChange={(e) => setIsGuestLecture(e.target.checked)} className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600"/>
                    <label htmlFor="guest-check" className={labelClasses}>Is this a guest lecture?</label>
                </div>
                {isGuestLecture && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 border rounded-md dark:border-gray-600 bg-gray-50 dark:bg-gray-700">
                        <div>
                        ...
                        <div className="md:col-span-2">
                            ...
                            <textarea id="guestDetails" name="guestDetails" placeholder="Brief details about the guest" rows="2" className={inputClasses}></textarea>
